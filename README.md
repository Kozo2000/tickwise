...
Tickwiseは。売買シグナル生成ツールではなく、ユーザー自身が分析するための補助エンジンです。
...
Tickwise は投資助言を目的としたツールではありません。
本ツールが提供するテクニカル指標・スコアリング・ニュース要約は、一般的な情報提供のみを目的としています。
特定の銘柄の売買や投資判断を推奨するものではありません。

Tickwise の利用により生じたいかなる損失についても、開発者は一切の責任を負いません。
投資判断は必ず利用者自身の判断と責任で行ってください。

・Tickwise は“投資判断を代替しません

・個別銘柄の助言を意図しません

・自動売買や売買シグナルの提供者ではありません

Tickwise は証券会社や取引所の公式データ提供サービスではありません。
株価データの正確性・遅延・欠損については保証しません。

# 17. ニュースデータの取り扱いについて（Brave News API）

Tickwise は Brave News API / Brave Search API を用いて、公開ニュースのメタ情報を取得します。
本ツールでは、Brave API が提供する タイトル・概要・URL などのメタデータのみ を利用し、以下の点に配慮しています。

● 1. 記事本文の取得・保存・再配布は行いません

Tickwise はニュース本文をスクレイピング・保存・再配布することはありません。
API が返すメタ情報のみを使用します。

● 2. 著作権保護と利用規約の尊重

ニュース内容の権利は各メディア企業に帰属します。
記事本文の閲覧は、必ずリンク先サイトの利用規約に従ってください。

● 3. ニュースは投資助言の材料として扱いません

ニュースは “市場理解の参考情報” として扱います。
Brave API が提供する内容の正確性・網羅性は保証されません。

● 4. Brave API の利用規約に準拠しています

Tickwise は Brave 検索サービスの利用ポリシーに準拠して動作しています：
https://docs.search.brave.com/