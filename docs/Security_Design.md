...
#### 注意事項　APIキー オプションについて
コマンドラインからのbrave_api_keyおよびopenai_api_keyオプションで本プログラムを動作させることは推奨しておりません。理由として、コマンドラインから実行するとコマンドヒストリに各種APIキー情報が記載されてしまい、何かの侵害が発生した場合、コマンドヒストリ情報からKEY情報が奪われることが想定できます。

本プログラムではenvファイルが不在でも動作させることを意図して本機能を残しています。ご利用される場合は上記ご理解のもとお願いします。また、将来的にこのコマンドオプションは無くなる可能性があります。
