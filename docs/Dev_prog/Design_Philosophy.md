...
### なぜコマンドライン（CLI）なのか
GUIアプリケーションではなくCLIという形態を選択したのには、アーキテクチャ上の明確な意図があります。

- **UXとエンジンの分離**: UIの表現層（プレゼンテーション）と計算ロジック（エンジン）を疎結合に保つ設計を目指しました。CLIというインターフェースを介することで、エンジン部分の純粋性を維持しています。
- **高い外部連携性**: シェル上でのパイプライン処理や他のスクリプトとの組み合わせなど、「道具としての応用力」を最大化するためです。他のアプリケーションのバックエンドとして組み込む際にも、コマンドラインは最も普遍的で強力な連携手段となります。
- **高速なネイティブパフォーマンス**: 外部ツールとの頻繁な連携や大規模な数値演算を想定し、仲介レイヤーのオーバーヘッドが極めて少ないネイティブアプリケーションであることを重視しました。
