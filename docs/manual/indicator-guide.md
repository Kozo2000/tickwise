# Tickwise 指標・スコア解説ガイド

> [!IMPORTANT]
> **⚠️ 免責事項**
> 本ドキュメントに記載されている指標の解釈やスコアリングの基準は、あくまで Tickwise の設計思想を説明するための**一例**であり、将来の利益を保証したり、特定の投資行動を推奨したりするものではありません。
> 実際の投資にあたっては、**必ずご自身の判断と責任において行ってください。**
...
|50% ± eps	|中央付近・迷い	|0|
|38.2% より上	|押しが浅い（強い）	|+2|
|50% + eps 〜 38.2% 未満	|戻り途中の強含み	|+1|
|61.8% 未満	|押しが深い（弱い）	|-2|
|61.8% 〜 50% - eps 未満	|下落寄りの弱含み	|-1|

- **eps (許容値)**: 0.50
    - 50.0% 水準の周辺 ±0.50 の範囲を「中立（0）」として扱う。
- ※ 境界に乗った場合でも、内部ロジックにより「必ず + / - どちらか」に倒すように丸め処理を行う設計。
...
- **計算エンジン**: **オリジナル計算**（Typical Price × `ta::SMA`）
1) Typical Price の算出
...
|-0.5% ～ +0.5%	|ほぼ同水準（中立）	|0|
|-0.5% 未満	|価格が平均よりやや下	|-1|
|-2.0% 未満	|価格が平均より明確に下	|-2|

- ※ 乖離率 = (現在値 - VWAP) / VWAP * 100
- ※ 数値は「勢い」ではなく 位置関係の目安
...
|diff の範囲	|解釈（参考）	|スコア|
|---	|---	|---|  
|2.0 超	|強い上方向（ゴールデンクロス）	|+2|
|0.5 超	|やや上方向	|+1|
|-0.5 ～ +0.5	|同値圏（拮抗）	|0|
|-0.5 未満	|やや下方向	|-1|
|-2.0 未満	|強い下方向（デッドクロス）	|-2|

#### 補足：乖離率（gap_ratio）による詳細表示
Tickwise は、スコアとは別に「転換線と基準線の乖離率」を計算し、
表示時に以下の補助情報を出す。
- **乖離 1.0% 未満**: 「接近状態でありトレンド確定には弱い」
- **乖離 5.0% 超**: 「大幅な乖離（乖離しすぎ）の可能性」

これにより、単なるクロスの有無だけでなく「勢いの過熱度」も視覚的に把握できるようになっている。
...
Tickwise では、計算の正確性と透明性を担保するため、Rust の標準的なテクニカル分析ライブラリである `ta` クレートの使用箇所と、Tickwise 独自の設計に基づいたオリジナル計算箇所を使い分け、明示しています。
...
Tickwise は、これまで解説した個別の指標スコアを合成し、最終的な投資判断の「目安」を算出する。
...
### 3. 正規化（ゲージ変換）
合計スコアを、視覚的に分かりやすい段階的な「ゲージ」に変換する。
Tickwise では、最終的な出力を以下の 5 段階（または数値）で表現する。

|ゲージ表示	|判断の目安|
|---	|---|
|高度過熱・強い売りシグナル	|複数の指標で売り傾向が濃厚|
|売り優勢	|全体として下落方向の圧力が強い|
|中立	|方向感が乏しい、または買い・売りが拮抗|
|買い優勢	|全体として上昇方向の圧力が強い|
|絶好の買い場・強い買いシグナル	|複数の指標で買い傾向が濃厚|

### 4. 判断の柔軟性
Tickwise は「この数値なら必ず買え」というブラックボックスなツールではない。
...
> - 戦略レシピを活用し、自分の投資スタイルに合った指標の組み合わせを見つけることが、Tickwise を使いこなす近道となる。
