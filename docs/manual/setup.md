...
README とは役割を分け> [!WARNING]
> **APIキーの取り扱いについて**
> APIキーは重要な個人情報です。絶対に第三者に公開しないでください。`tickwise.env` はローカル環境でのみ管理し、Git 等の公開設定には含めないよう注意してください。

### 5-1. ファイルの作成
[**Binary/tickwise.env.sample**](../../Binary/tickwise.env.sample) をコピーし、ファイル名を `tickwise.env` に変更します。
ろまで」に集中して解説します。
...
Tickwise > > 現在のバージョンでは、`tickwise.env` は「コマンドを実行しているディレクトリ（カレントディレクトリ）」に置かれている必要があります。パス（PATH）を通した場所から実行する場合も、設定情報が必要な場合はその作業ディレクトリに `tickwise.env` を配置してください。*tickwise.env の場所について**
> 現在のバージョンでは、`tickwise.env` は「コマンドを実行しているディレクトリ（カレントディレクトリ）」に置かれている必要があります。パス（PATH）を通した場所から実行する場合も、設定情報が必要な場合はその作業ディレクトリに `tickwise.env` を配置してください。行するために必要な準備は次の 4 点です。
...
- **`tickwise.env` が認識されない**
  - ファイル名が正しいか、また、**「今コマンドを入力しているその場所（カレントディレクトリ）」**に置かれているかを確認してください。プログラム本体（バイナリ）と同じ場所にファイルを置いても、別のフォルダから実行した場合は読み込まれません。その場合はファイルがあるフォルダに `cd` コマンドで移動するか、実行したい場所にファイルをコピーしてください。
- **通信エラーが出る**
