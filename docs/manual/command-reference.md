...
| `--alias-csv` | - | 会社名エイリアス（銘柄リスト）のCSVパス | String | - | `ALIAS_CSV` |
...
> [!IMPORTANT]
> `tickwise.env` は、**コマンドを実行しているディレクトリ（カレントディレクトリ）**からのみ読み込まれます。バイナリと同じ場所に置いても、別のディレクトリから実行した場合は参照されませんのでご注意ください。
