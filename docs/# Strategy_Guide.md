...
#### 例）　逆張りとブレイク狙いの違い（同一銘柄・同一日時の比較）

同じ銘柄・同じ日時でも、狙い（レシピ）を変えると評価軸が変わり、結論も変わる。

以下はソフトバンクグループ（9984.T）を、
「逆張り」と「ブレイク狙い」の 2 つのレシピで評価した例である。

 観点    	        逆張り  	       ブレイク狙い

主役となる指標	    RSI / Bollinger	    ROC / ADX

今回の相場評価      条件未達	         条件未達

総合スコアの傾向	買い寄り（69%）	      中立（-9%）
...
逆張り
- → 「売られ過ぎ」「収束」といった条件が揃っていないため、
買いは検討できるが、積極的に踏み込む段階ではない。
